...
每种强大的语言都有三种特性：
- __原始的表达式和语句__，代表语言提供的最简单的构建块
- __组合的手段__，从简单的元素构建复合元素
- __抽象的手段__，复合元素可以命名和操纵如同单位

在编程中，我们处理两种元素：**函数**和**数据**
...
__New environment Features__：Assignment and user-defined function definition. (想像在创造Python，为Python增加了一个新特性)(assignment：翻译为赋值)  
新环境特点：赋值和用户定义的函数定义。

## 1.3.2 调用用户定义的函数
求值一个调用表达式，其运算符命名用户定义的函数，Python解释器按照相似的过程对于使用内建运算符函数求值的表达式。也就是说，解释器对操作数表达式求值，然后将命名函数应用到结果参数。

应用用户定义函数的行为引入了第二个局部栈帧，这个栈帧只能被该函数访问。应用用户定义函数的一些理由：
1. 将参数绑定到新的局部栈帧中函数形参的名称上
2. 从该栈帧开始到全局栈帧结束，在这个环境中求函数体的值。

求值主体的环境由两个栈帧组成：首先是局部栈帧包含参数绑定，然后是全局栈帧包含其他所有内容。 函数应用程序的每个实例都有自己独立的局部栈帧。
...
下一步，Python调用`sum_squares`,引入局部栈帧x绑定5，y绑定12。

![](http://i63.tinypic.com/jz8dwi.png)

在该图中，局部栈帧指向其继承者，全局栈帧。 所有局部帧必须指向一个前任，这些链接定义了当前环境的帧序列。

`sum_squares`的主体包含这个调用表达式。

![](http://i63.tinypic.com/wafupd.png)

运算符子表达式`add`在全局栈帧中查找，绑定到内置的加法函数。在使用加法前，必须依次计算两个操作数子表达式。在下图中，这个局部栈帧记为`A`,同样使用标签`A`替换指向这个栈帧的箭头。

在`operand 0`中，`square`是全局栈帧中用户定义的函数，而`x`在局部栈帧是5。Python通过引入另一个x绑定到5的局部栈帧来调用`square(5)`。
...
## 1.3.4 局部名称
函数实现的一个细节是，实现者选择的函数形参名不应该影响函数的行为。因此，下面两个函数应具有相同的行为。
```python
>>> def square(x):
        return mul(x, x)
>>> def square(y):
        return mul(y, y)
```
这条原则--函数的意义与其作者选择的参数名称无关--对编程语言有重要的影响。最简单的影响是函数的参数名称必须留在函数体的局部域内。

如果参数不在各自函数体内部，那么`square`函数中的`x`可能会与`sum_squares`函数中的`x`混淆。确切的说，并非如此：不同局部栈帧中绑定的`x`是无关的。我们的计算模型是精心设计的以确保这种独立性。

我们说一个局部名称的*作用域*仅限于定义它的函数体内。当一个名称不可被访问时，就超出了它的作用域了。这种作用域行为并不是我们模型中的一个新东西，而是“环境”工作机制的结果。
...
## 1.4 实用指南：函数的艺术
函数是所有程序必不可少的组成部分，无论大小，在编程语言中充当了我们表达计算过程的主要媒介。所以编写好的函数，应遵循下面的原则：
- 每个函数应该只有一个明确的任务。这个任务应该用一个简短的名字来识别，并且可以用一行文字来解释。序列执行多个任务的函数应该分为多个函数。
- *不要重复自己*是软件工程的中心原则。所谓的DRY原则规定，多段代码不应描述冗余逻辑。 相反，这个逻辑应该实现一次，给定一个名称，并且应用多次。 如果您发现自己复制并粘贴了一个代码块，你可能找到了一个函数抽象的机会。
- 函数定义要具有普适性。`square`不在Python标准库中，因为它是`pow`函数的特例，它可以求数字的任意次方。
